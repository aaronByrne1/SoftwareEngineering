...
public class DAGLcaTest {
	...
	DagLca acyclic =new DagLca(8);//create an acyclic graph
	DagLca cycle = new DagLca(9);//create a cycle graph
	DagLca directAcyclic = new DagLca(9);//creates an acyclic graph
	
	//testing the set up of the constructor
	@Test(expected = IllegalArgumentException.class)//must be non-negative
	public void testDAG() {
		DagLca negativeTest = new DagLca(-2);//will throw IllegalException if out of bounds
	}
	...
	public void testCycle(){
		...
		DagLca emptyGraph = new DagLca(0);
